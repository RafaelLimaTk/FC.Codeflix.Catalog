...
public class CreateCastMemberApiTest : IDisposable
{
    ...
    public async Task Create()
    {
        ...
        var (response, output) =
            await _fixture.ApiClient.Post<ApiResponse<CastMemberModelResponse>>(
                "/cast_members",
                new CreateCastMemberRequest(example.Name, example.Type)
            );
    ...
    public async Task ThrowWhenNameIsEmpty()
    {
        ...
        var (response, output) =
            await _fixture.ApiClient.Post<ProblemDetails>(
                "/cast_members",
                new CreateCastMemberRequest("", example.Type)
            );
